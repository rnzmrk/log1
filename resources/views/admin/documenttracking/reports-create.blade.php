...
<div class="container-fluid px-6 py-8">
    ...
    <nav class="flex mb-6" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    <i class='bx bx-chevron-right text-gray-400'></i>
                    <span class="ml-1 text-gray-500 md:ml-2">Generate New Report</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <form method="POST" action="{{ route('document-reports.store') }}" class="p-6 space-y-6">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="report_type" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            <option value="Summary" {{ old('report_type') === 'Summary' ? 'selected' : '' }}>Summary Reports</option>
                            <option value="Storage" {{ old('report_type') === 'Storage' ? 'selected' : '' }}>Storage Reports</option>
                            <option value="Access" {{ old('report_type') === 'Access' ? 'selected' : '' }}>Access Logs</option>
                            <option value="Compliance" {{ old('report_type') === 'Compliance' ? 'selected' : '' }}>Compliance Reports</option>
                            <option value="Upload" {{ old('report_type') === 'Upload' ? 'selected' : '' }}>Upload Activity</option>
                            <option value="Document Requests" {{ old('report_type') === 'Document Requests' ? 'selected' : '' }}>Document Requests</option>
                        </select>
                        @error('report_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <!-- Generated By and Report Date -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Generated By *</label>
                        <input type="text" 
                               name="generated_by"
                               value="{{ old('generated_by', auth()->user()->name ?? 'Admin User') }}"
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="parameters" 
                              ...
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('parameters') }}</textarea>
                    @error('parameters')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
