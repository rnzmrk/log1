...
<div class="container-fluid px-6 py-8">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <form method="POST" action="{{ route('logistics-reports.store') }}" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="report_type" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        ...
                        <option value="Delivery" {{ old('report_type') === 'Delivery' ? 'selected' : '' }}>Delivery Report</option>
                        <option value="Vehicle" {{ old('report_type') === 'Vehicle' ? 'selected' : '' }}>Vehicle Report</option>
                        <option value="Project" {{ old('report_type') === 'Project' ? 'selected' : '' }}>Project Report</option>
                        <option value="Performance" {{ old('report_type') === 'Performance' ? 'selected' : '' }}>Performance Report</option>
                        ...
                        <option value="Maintenance" {{ old('report_type') === 'Maintenance' ? 'selected' : '' }}>Maintenance Report</option>
                    </select>
                    @error('report_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-6">
                ...
                <textarea name="description" 
                          rows="4" 
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                          placeholder="Enter report description...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end gap-3 mt-8">
                ...
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-6 rounded-lg transition-colors">
                    <i class='bx bx-file text-lg mr-2'></i>
                    Generate Report
                </button>
            </div>
        </form>
