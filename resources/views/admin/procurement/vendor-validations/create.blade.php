...
<div class="container-fluid px-6 py-8">
    ...
    <nav class="flex mb-6" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    <i class='bx bx-chevron-right text-gray-400'></i>
                    <span class="ml-1 text-gray-500 md:ml-2">Add Validation</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Add Validation</h1>
            <p class="text-gray-600 mt-1">Add document validation for {{ $supplier->name }}</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <form method="POST" action="{{ route('vendors.validations.store', $supplier->id) }}">
            @csrf
                        ...
                        <div>
                            ...
                            <select name="validation_type" 
                                ...
                                @foreach($validationTypes as $key => $type)
                                    <option value="{{ $key }}" {{ old('validation_type') === $key ? 'selected' : '' }}>{{ $type }}</option>
                                @endforeach
                            </select>
