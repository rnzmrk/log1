...
<div class="container-fluid px-6 py-8">
    ...
    <nav class="flex mb-6" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    <i class='bx bx-chevron-right text-gray-400'></i>
                    <span class="ml-1 text-gray-500 md:ml-2">Edit Verification</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Edit Verification</h1>
            <p class="text-gray-600 mt-1">Update verification process for {{ $supplier->name }}</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <form method="POST" action="{{ route('vendors.verifications.update', [$supplier->id, $verification->id]) }}">
            @csrf
            @method('PUT')
            <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="verification_type" 
                                ...
                                @foreach($verificationTypes as $key => $type)
                                    <option value="{{ $key }}" {{ old('verification_type', $verification->verification_type) === $key ? 'selected' : '' }}>{{ $type }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" 
                                   ...
                                   value="{{ old('verification_date', $verification->verification_date ? $verification->verification_date->format('Y-m-d') : '') }}"
                                   required
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        ...
                        <div>
                            ...
                            <select name="status" 
                                ...
                                <option value="scheduled" {{ old('status', $verification->status) === 'scheduled' ? 'selected' : '' }}>Scheduled</option>
                                <option value="pending" {{ old('status', $verification->status) === 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="passed" {{ old('status', $verification->status) === 'passed' ? 'selected' : '' }}>Passed</option>
                                <option value="failed" {{ old('status', $verification->status) === 'failed' ? 'selected' : '' }}>Failed</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   name="score" 
                                   id="score" 
                                   value="{{ old('score', $verification->score) }}"
                                   min="0" 
                                   max="100"
                                   placeholder="e.g., 85"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" 
                                   name="report_path" 
                                   id="report_path" 
                                   value="{{ old('report_path', $verification->report_path) }}"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="findings" 
                                      ...
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('findings', $verification->findings) }}</textarea>
                            @error('findings')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="recommendations" 
                                      ...
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('recommendations', $verification->recommendations) }}</textarea>
                ...
                @if($verification->verified_by)
                    <div class="bg-green-50 p-4 rounded-lg">
                        <h4 class="text-sm font-medium text-green-900 mb-2">Verification Information</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                                <span class="text-green-700">Verified By:</span>
                                <span class="font-medium text-green-900 ml-2">{{ $verification->verifier ? $verification->verifier->name : 'Unknown' }}</span>
                            </div>
                            <div>
                                <span class="text-green-700">Grade:</span>
                                <span class="font-medium text-green-900 ml-2">{{ $verification->grade }}</span>
                            </div>
                        </div>
                    </div>
                @endif
